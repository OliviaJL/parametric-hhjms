...
pre_estimator <- function(data, job, instance, distribution, ...){
  ...
  if (distribution == 'No_distribution'){
    ...
    ifVelse <- function(x=vector()){
      for(i in length(x)){ x[i] = ifelse(x[i]>0, x[i], 1e-6) }
      return(x)
    }
    survObject = list(
      fm = obs_time ~ base+b11+b21,
...
JM_summary <- function(testjm, newSD = FALSE, digits = 3){
  ...
  if (newSD == FALSE){
    ...
    Sd =  numeric(length(testjm$new_sd))
    new_sd = as.numeric(testjm$new_sd)
    for(i in 1:length(Sd)){Sd[i] = ifelse(is.na(new_sd[i]), testjm$fixedsd[i], testjm$new_sd[i])}
  }
  ...
  Coeff = as.data.frame(Coeff) # transform to data frame to display row names
